...
class Mywindow(QMainWindow,Ui_MainWindow):
    ...
    def generate_video_with_classfication(self,model, model_input_size, video_name_or_camera, cam_weights, generate_video_name='output.avi'):
        ...
        self.cap = cv2.VideoCapture(video_name_or_camera)
        video_width = self.cap.get(cv2.CAP_PROP_FRAME_WIDTH)
        video_high = self.cap.get(cv2.CAP_PROP_FRAME_HEIGHT)
        ...
        while True:
            ...
            """ 5. show video """
            if(self.tag1==0):
                cv2.imshow('image1', showBigImage)
    ...
    def show_cam(self):
        cam_model1, cam_weights = self.cam_model(resnet50.ResNet50, res_input_shape, resnet50.preprocess_input, 10, 'my_model_weights.h5')
        self.generate_video_with_classfication(cam_model1, (224, 224), 'train_deal_video.avi', cam_weights, generate_video_name='output.avi')
        #self.timer_camera.start(10)
